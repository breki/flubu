...
namespace Flubu.Builds.Tasks.NuGetTasks
{
    public class FindNuGetPackageInUserRepositoryTask : TaskBase
    {
        ...
        {
            get { return string.Format(CultureInfo.InvariantCulture, "Find NuGet package {0} in user repository", packageId); }
        }
        ...
        protected override void DoExecute(ITaskContext context)
        {
                ...
                string.Format (CultureInfo.InvariantCulture, "{0}.*", packageId)))
            {
                ...
                if (packageVersion == null || version > packageVersion)
                {
                    ...
                    packageDirectory = Path.Combine(DownloadNugetPackageInUserRepositoryTask.UserProfileNuGetPackagesDir, directory);
                }
            }

            if (packageVersion != null)
                context.WriteDebug("Found NuGet package {0} version {1} in user repository", packageId, packageVersion);
            else
                context.WriteDebug ("No NuGet package {0} in user repository", packageId);
        }
