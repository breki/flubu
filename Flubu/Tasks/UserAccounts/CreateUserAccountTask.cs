...
namespace Flubu.Tasks.UserAccounts
{
    ...
    public class CreateUserAccountTask : TaskBase
    {
        ...
        {
            get { return String.Format(System.Globalization.CultureInfo.InvariantCulture, "Create user account '{0}'", userName); }
        }

        public CreateUserAccountTask(CreateUserAccountMode mode, string userName, string password, string userDescription)
        {
            this.mode = mode;
        ...
        public static void Execute(ITaskContext environment, CreateUserAccountMode mode, string userName, string password, string userDescription)
        {
            ...
            using (DirectoryEntry rootDirectoryEntry = new DirectoryEntry("WinNT://" + Environment.MachineName + ",computer"))
            {
                ...
                try
                {
                    ...
                    try
                    {
                        ...
                        {
                            context.WriteInfo("User '{0}' already exists, doing nothing.", userName);
                            return;
                        }
                        else if (mode == CreateUserAccountMode.FailIfAlreadyExists)
                        {
                            throw new TaskExecutionException(String.Format(System.Globalization.CultureInfo.InvariantCulture, "User '{0}' already exists.", userName));
                        }
        ...
        private readonly CreateUserAccountMode mode;
